...
class ReportRepository extends base_repository_1.BaseRepository {
    ...
    async getOrderReport(startDate, endDate) {
        ...
        const [totalOrders, completed, pending, cancelled, returned, paymentStats] = await Promise.all([
            ...
            this.prisma.order.count({ where: { ...where, status: client_1.OrderStatus.RETURNED } }),
        ...
        return {
            ...
            paymentStatus: paymentStats.map((p) => ({
                status: p.status,
                count: p._count._all,
            })),
        };
    ...
    async getCustomerReport() {
        ...
        const topCustomerDetails = await Promise.all(topSpendCustomers.map(async (item) => {
    ...
    async getInventoryReport() {
        ...
        const totalInventoryValue = inventories.reduce((acc, inv) => acc + inv.availableStock * (inv.product?.price ? Number(inv.product.price) : 0), 0);
